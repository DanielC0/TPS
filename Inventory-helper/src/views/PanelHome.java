...
public class PanelHome extends javax.swing.JPanel {
    ...
    WDefaultTableModel modeltbOrder = null;
        			...
        			if (cantidad != 0 ) {
            			...
            			if(diccionarioFactura.get(idProd) != null ) {
            				if((cantidad + diccionarioFactura.get(idProd)) <= cantLimite  ) {
            					...
            					modeltbOrder.addRow(new Object[] {tableStock.getValueAt(rwselect,0).toString(),tableStock.getValueAt(rwselect,1).toString(), txtQuantity.getText(), 
        				...
        				JOptionPane.showMessageDialog(null, "Intenta vender 0 cantiades","Alerta",JOptionPane.WARNING_MESSAGE);
        			}
...
//    	System.out.println("Rows " + tableOrder.getModel().getRowCount());
    	if (tableOrder.getModel().getRowCount() > 0) {
    		
    		double totalAmount = Double.valueOf( labelTotalAmount.getText());
    		Date fecha = new Date();
    		MovementDAO.create(new Movement(0, "out", totalAmount, fecha));
    		int lastM = MovementDAO.readLastM();
    		
    		for (int i = 0; i < tableOrder.getModel().getRowCount(); i++) {
				// crearmovimiento
    			// ultimo id
    			try {
					double cant = Double.valueOf(tableOrder.getValueAt(i, 2).toString());
					double precioT = Double.valueOf(tableOrder.getValueAt(i, 3).toString());
					double precioU = precioT / cant ;
...
//					// create dealing
					DealingDAO.create(new Dealing(0, Integer.valueOf(tableOrder.getValueAt(i, 0).toString()), cant, precioU, lastM));
//					
//					//update product
					ProductDAO.update(Integer.valueOf(tableOrder.getValueAt(i, 0).toString()), diccionarioStock.get(tableOrder.getValueAt(i, 0).toString()) - cant );
					diccionarioStock.put(tableOrder.getValueAt(i, 0).toString(), (diccionarioStock.get(tableOrder.getValueAt(i, 0).toString()) - cant ));
					this.getData();
					diccionarioFactura.remove(tableOrder.getValueAt(i, 0).toString());
				} catch (Exception e) {
    ...
    java.util.Hashtable<String, Double> diccionarioStock = new java.util.Hashtable<String, Double>();
...
//		category.name from product join category on product.category = category.id
		...
		WDefaultTableModel modeltbStock = new WDefaultTableModel(
		...
		try {
			...
			diccionarioStock = new java.util.Hashtable<String, Double>();
			// load products
			while (rs.next()) {
				modeltbStock.addRow( new Object[] {rs.getInt(1),rs.getString(2), rs.getDouble(3), rs.getDouble(4)});
				diccionarioStock.put(String.valueOf(rs.getInt(1)), rs.getDouble(3));
			}
		...
		tableStock.setModel(modeltbStock);
